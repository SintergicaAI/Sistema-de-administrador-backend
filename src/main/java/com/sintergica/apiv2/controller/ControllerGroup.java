...
public class ControllerGroup {
    ...
    public ResponseEntity<HashMap<String,String>> addNewClientToGroup(
        ...
        if(currentGroup != null &&
            ...
            return ResponseEntity.ok(new HashMap<>(){{put("Exito", "Usuario agregado al grupo");}});
        }

        return ResponseEntity.badRequest().body(new HashMap<>(){{put("Sin exito", "Uno de los campos es nulo o la empresa no esta asociada con el usuario y grupo");}});
    }
    ...
    public ResponseEntity<HashMap<String, Object>> addNewGroup(
        ...
        if (company.isEmpty()) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new HashMap<>(){{put("Sin exito","La compañia no existe");}});
        }
        ...
        return ResponseEntity.ok(new HashMap<>(){{put("Exito", "Grupo agregado");}});
    ...
    public ResponseEntity<Group> getGroupByUUID(
            @RequestParam UUID groupUUID
    )
    {
        return groupRepository.findById(groupUUID).map(ResponseEntity::ok).orElse(ResponseEntity.notFound().build());
    }
